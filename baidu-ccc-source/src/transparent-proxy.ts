...
interface QuestionDetection {
  type: 'question' | 'choice' | 'yesno' | 'prompt';
  text: string;
  raw: string;
}
...
class TransparentProxy {
  ...
  private inputWatchInterval: NodeJS.Timeout | null = null;
  ...
  async start(): Promise<void> {
    ...
    this.proc.on('exit', (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      // 停止监听
      if (this.inputWatchInterval) clearInterval(this.inputWatchInterval);
  ...
  private handleOutput(data: Buffer, source: 'stdout' | 'stderr'): void {
  ...
  private detectQuestion(text: string): QuestionDetection | null {
    // 多种问题模式检测
    const patterns: Array<{ regex: RegExp; type: QuestionDetection['type'] }> = [
      // 模式1: ? 问题文本
      { regex: /\?\s*([^\n]+?)[:：]\s*$/m, type: 'question' },
  ...
  private updateState(state: Partial<ProxyState>): void {
    const currentState = this.readState();
    const newState = { ...currentState, ...state };
    fs.writeFileSync(STATE_FILE, JSON.stringify(newState, null, 2));
  }

  private readState(): ProxyState {
    try {
